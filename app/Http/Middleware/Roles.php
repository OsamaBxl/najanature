        ...
        if (auth()->check()) {
            if (!in_array($route[0], $roleRoutes)) {
                ...
                if ($route[0] != auth()->user()->roles[0]->allowed_route) {
                    $path = $route[0] == auth()->user()->roles[0]->allowed_route ? $route[0] . '.login' : '' . auth()->user()->role[0]->allowed_route . '.index';
            ...
            $path = $route[0] != '' ? $routeDestination : auth()->user()->roles[0]->allowed_routes . '.index';
            return redirect()->route($path);
        }
    }
}